        ...
        while True:
            ...
            if method == "initialize":
                ...
                try:
                    ...
                    # Result is likely a list of Content objects (TextContent, ImageContent, etc)
                    # We need to serialize them
                    content_list = []
                    
                    # Handle tuple return if applicable (based on inspection it returned a tuple)
                    ...
                    if isinstance(actual_result, list):
                        for item in actual_result:
                         ...
                         content_list.append({"type": "text", "text": str(actual_result)})
...
async def start_tcp_server(mcp_instance, host="0.0.0.0", port=8000):
    server = await asyncio.start_server(
        lambda r, w: handle_client(r, w, mcp_instance), 
        host, 
        port
    )

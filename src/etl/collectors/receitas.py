...
class RevenueCollector(BaseCollector):
        ...
        for month in range(1, 13):
            ...
            if data:
                ...
                if 'rsp' in data and '_content' in data['rsp']:
                        ...
                        content = data.get('data') or data.get('rows') or data.get('balancete_receita_orcamentaria')
    ...
    def save(self, batch_data, municipio_id, year, month_ref):
        ...
        for i, item in enumerate(batch_data):
            ...
            """, (
                rec_id, municipio_id, str(year), month_ref,
                item.get('codigo_orgao'), item.get('codigo_unidade_orcamentaria'),
                item.get('codigo_receita'), item.get('descricao_receita'), 
                item.get('valor_previsto_arrecadacao'), item.get('valor_arrecadado_no_mes'), 
                json.dumps(item)
            ))
            count += 1
        conn.commit()

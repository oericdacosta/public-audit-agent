...
class TestFetchJson:
    ...
    def setup_method(self) -> None:
        ...
        TCEClient._session = None

    @patch("src.etl.client.get_settings")
    @patch("src.etl.client.requests.Session")
    def test_returns_json_on_success(
        self, mock_session_cls: MagicMock, mock_settings: MagicMock
    ) -> None:
        """Should return parsed JSON on successful request."""
        mock_settings.return_value = {"tce": {}}

        # Mock Session instance and its get method
    ...
    def test_returns_none_on_request_exception(
        ...
        mock_session = mock_session_cls.return_value
        # Use side_effect on the session.get method
